...
export default function App() {
    const { data, isLoading, isError, refetch } = useGetStudentsQuery(null, {
        ...
        //     return result;
        // }, // 用来指定useQuery返回的结果
        // pollingInterval:500 // 设置轮询的间隔，单位毫秒 如果为0则表示不轮询 默认0
        // skip:true, // 设置是否跳过当前请求，默认false
        // refetchOnMountOrArgChange: false, // 设置是否每次都重新加载数据 false正常使用缓存，true每次都重载数据  数字，数据缓存的时间（秒）默认false
        refetchOnFocus: false, // 是否在重新获取焦点时重载数据 默认false
        refetchOnReconnect: true, // 是否在重新连接后重载数据 默认true
    });
    return (
        <>
